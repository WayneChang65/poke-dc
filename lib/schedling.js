...
function _setCyclicTimer_Do_Everyday(_event, _bot,
	...
	setTimeout(function () {
		...
		setInterval(function () {
			console.log('');
			fmlog('crawler_msg', ['SCHEDULING', '每24小時啟動。',
				'-'.repeat(26), ' ', basic_f.getCurrentDateTime()
			]);
			_runningFunc(_event, _bot);
		}, 1000 * 60 * 60 * 24); // 1天
...
function _setCyclicTimer_Do_EveryInterval(_event, _bot,
	_runningFunc, _time_ms, _channel, _consoleString1, _consoleString2) {
	fmlog('sys_msg', ['SCHEDULING',
		basic_f.getCurrentDateTime() + '---【TDiff = ' + _time_ms + '】---' + _consoleString2
	]);
	setInterval(function () {
		console.log('');
		fmlog('crawler_msg', ['SCHEDULING', '每' + (_time_ms / 1000 / 60).toString() + '分鐘啟動。',
			'-'.repeat(26), ' ', basic_f.getCurrentDateTime()
		]);
		_runningFunc(_event, _bot, _time_ms, _channel);
	}, _time_ms);
}
...
//////////////  Module Exports //////////////////
module.exports = {
	setCyclicTimer_Do_Every_10_days: _setCyclicTimer_Do_Every_10_days,
	setCyclicTimer_Do_Everyday: _setCyclicTimer_Do_Everyday,
	setCyclicTimer_Do_EveryInterval: _setCyclicTimer_Do_EveryInterval
};